...
// The 'jodit' import below seems unused and can be safely removed if not used elsewhere
// import { orderedList } from 'jodit/esm/plugins/ordered-list/ordered-list';

const navItems = [
  { name: "Dashboard", href: "/vendor", icon: LayoutDashboard },
  // {
  //   name: "Manage New Reservation",
  //   href: "/vendor/manage-new-reservation",
  //   icon: FileText,
  // },
  ...
  { name: "My Menu", href: "/vendor/my-menu", icon: PanelTopOpen },
  // { name: "Events", href: "/vendor/events", icon: CalendarDays },
  // {
  //   name: "Event Registration",
  //   href: "/vendor/event-registration",
  //   icon: CalendarCog,
  // },
  // {
  //   name: "Banner Management",
  //   href: "/vendor/banner-management",
  //   icon: ImageIcon,
  // },
  // { name: "Promotion Setup", href: "/vendor/promotion-setup", icon: Megaphone },
  // {
  //   name: "Customer Reviews",
  //   href: "/vendor/customer-reviews",
  //   icon: StarIcon,
  // },
  { name: "Withdraws", href: "/vendor/withdraws", icon: BanknoteArrowDown },
  { name: "Settings", href: "/vendor/settings", icon: Settings },
];

const Sidebar = ({ isOpen, setIsOpen }) => {
  const pathname = usePathname();
  const [isSettingsOpen, setIsSettingsOpen] = useState(false); // State to manage settings dropdown
  ...
  return (
    <>
      <aside
        className={`fixed top-0 overflow-hidden left-0 h-full bg-white text-black shadow-lg z-50 transform transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="flex flex-col h-full justify-between border-r border-[#D6D6D6]">
          ...
          {/* Navigation Items */}
          <nav className="mt-4 space-y-6 flex-grow overflow-y-auto hide-scrollbar">
            {navItems.map(({ name, href, icon: Icon }) => {
              ...
              if (name === "Settings") {
                return (
                  <div key={name}>
                    <a
                      href="#" // Using href="#" for a clickable but non-navigating link
                      onClick={handleSettingsClick}
                      className={`flex items-center px-4 w-[218px] mx-auto py-2 transition-all rounded ${
                        isActive || pathname.startsWith("/vendor/settings/") // Check if current path starts with settings
                          ? "bg-[#B92921] text-white "
                          : "hover:bg-gray-200 text-black" // Adjusted hover and default text color
                      }`}
                    >
                    ...
                    >
                      <div className="ml-8 mt-2 space-y-2">
                        ...
                        <Link
                          href="/vendor/settings/edit-profile"
                          className={`flex items-center px-4 w-[180px] py-2 transition-all rounded ${
                            pathname === "/vendor/settings/edit-profile"
                              ? "bg-[#B92921] text-white "
                              : "hover:bg-gray-200 text-gray-700" // Adjusted hover and default text color
                          }`}
                        >
                          <UserPen className="w-4 h-4 mr-3" />{" "}
                          {/* Example icon for Edit Profile */}
                        ...
                        <Link
                          ...
                          className={`flex items-center px-4 w-[180px] py-2 transition-all rounded ${
                            pathname === "/vendor/settings/bank-details"
                              ? "bg-[#B92921] text-white "
                              : "hover:bg-gray-200 text-gray-700" // Adjusted hover and default text color
                          }`}
                        >
                          <CreditCard className="w-4 h-4 mr-3" />{" "}
                          {/* Example icon for Bank Details */}
                          <span className="font-normal text-[12px]">
                            Bank Details
                          </span>
                        </Link>
            ...
            <div className="border-t border-[#D6D6D6] pt-6 ">
              <button className="flex ml-9 gap-2 items-center text-red-500 hover:text-red-600">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
      ...
      {!isOpen && (
        <button
          ...
          className="fixed top-4 left-4 z-50 bg-white p-2 rounded-md shadow-lg flex items-center justify-center hover:bg-gray-200 transition" // Adjusted background for the open button
        >
